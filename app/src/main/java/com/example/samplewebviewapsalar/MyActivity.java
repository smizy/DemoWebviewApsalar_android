...
public class MyActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean useApsalarJSInterface = true;
        if (useApsalarJSInterface) {
            ...
            // handling custom url scheme version (need load Apsalar.js)
            mWebView.setWebViewClient( new WebViewClient() {
                private Activity _context = mContext;
                @Override
                public boolean shouldOverrideUrlLoading( WebView view, String url ) {

                    String urlScheme = Uri.parse(url).getScheme();

                    if (urlScheme.equals("apsalar")) {
                        Log.d("ApsalarURL:", url);
                        if (!Uri.parse(url).getHost().equals("event")) {
                            return true;
                        }
                        if (Uri.parse(url).getPath().equals("")) {
                            return true;
                        }
                        String eventName = Uri.parse(url).getLastPathSegment();
                        JSONObject args = new JSONObject(parseUrlParameters(url));
                    ...
                    Log.d("CallbackURL:", urlScheme);

                    // handle only internal files
                    //return !Uri.parse(url).getScheme().equals("file");
